...
VALUES
    -- Question 1 for C++
    (13, 'Which of the following is used to input/output in C++?', 'input/output.h', 'stdinputoutput.h', 'iostream.h', 'stdio.h', 'iostream.h', 4),
    
    -- Question 2 for C++
    (14, 'What is the correct syntax to declare a pointer in C++?', 'int* ptr;', 'pointer int ptr;', 'int ptr;', 'ptr int;', 'int* ptr;', 4);

INSERT INTO Challenges (Challenge_ID, Title, Description, Problem_ID, Answer)
VALUES
    -- Challenge 1
    (101, 'Basic Java Challenge', 'Test your knowledge of basic Java concepts.', 1, 'Java is an object-oriented programming language.'),
    
    -- Challenge 2
    (102, 'Intermediate Python Challenge', 'Solve intermediate-level Python problems.', 2, 'Python supports both procedural and object-oriented programming.');
...
--Other Constraints SQLs (Unique, Not Null, Check, …etc.)
---list out the SQL statements which creates other constraints for all tables---
-- Add CHECK constraint to Score column in 'User_Leaderboard' table
ALTER TABLE User_Leaderboard
ADD CONSTRAINT CHK_Score CHECK (Score >= 0);

-- Add NOT NULL constraint to Title column in 'quiz' table
ALTER TABLE quiz
...
-- add a check constraint on the users table to ensure that the Role 
--column only contains values "admin" or "regular user". 
...
-- Check constraint on Language_ID column
ALTER TABLE Problem
ADD CONSTRAINT CK_Problem_Language_ID CHECK (Language_ID > 0);

-- Unique constraint on Username column
ALTER TABLE users
ADD CONSTRAINT UQ_users_Username UNIQUE (Username);

-- Not Null constraint on  FirstName, LastName, Passwords, and Role columns

-- Not Null constraint on FirstName column
ALTER TABLE users
ALTER COLUMN FirstName VARCHAR(50) NOT NULL;

-- Not Null constraint on LastName column
ALTER TABLE users
ALTER COLUMN LastName VARCHAR(50) NOT NULL;

-- Not Null constraint on Passwords column
ALTER TABLE users
ALTER COLUMN Passwords VARCHAR(255) NOT NULL;

-- Not Null constraint on Role column
ALTER TABLE users
ALTER COLUMN Role VARCHAR(50) NOT NULL;

--Columns Management SQLs
---Mention the table(s) that you are going to add/delete columns to/from them---
---list out the ALTER TABLE statements to add/delete those column(s)---
ALTER TABLE users
ADD Phone VARCHAR(20); -- Adding a new column for phone number

ALTER TABLE quiz
ADD Time_Limit INT; -- Adding a new column for time limit in minutes
...
--presenation question 
select u.Username, COUNT(c.Challenge_ID)
from users u, Problem p, Challenges c, programmingLanguage pr
where u.User_ID = p.user_ID and p.Language_ID = pr.Language_ID and c.Problem_ID = p.Problem_ID and pr.Language_Name ='python'
group by u.Username;
